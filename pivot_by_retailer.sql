SELECT `champion_registry`.retailer,
...
from `champion_eiep1` inner join `champion_registry` using (icp) where `champion_registry`.icpstatus = 2 group by `champion_eiep1`.retailer
